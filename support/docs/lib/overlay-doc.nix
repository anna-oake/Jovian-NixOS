...
let
  ...
  documentDerivation =
    ...
    let
      toNixpkgsRelative =
        decl:
        if lib.hasPrefix (toString pkgsPath) (toString decl) then
          "<nixpkgs/${lib.removePrefix "/" (lib.removePrefix (toString pkgsPath) (toString decl))}>"
        else
          "(...)";
      toRelative =
        decl:
        if lib.hasPrefix (toString prefix) (toString decl) then
          ...
          (toNixpkgsRelative decl);
      url =
        if position == null || lib.hasPrefix "<nixpkgs" position then
          null
        else
          let
          ...
          in
          "${sourceLinkPrefix}/${subpath}#L${lineno}";
      position = if drv.meta ? "position" then toRelative drv.meta.position else null;
    in
    {
      entry_type = "package";
      inherit
    ...
    // optionalAttrs (drv ? meta.license) {
      license =
        if builtins.isList drv.meta.license then builtins.head drv.meta.license else drv.meta.license;
    };
  overlayData = mapAttrs (
    _: entry:
